h2. How to get jcom.hub's initialization notification?
...
In this example, we built a similar situation using three modules from Jamoma Modular core distribution ("jmod.qmetro":modules/qmetro.html that we will use as a clock, "jmod.10harmonics~":modules/10harmonics~.html that we will use as mini synthesizer and "jmod.output~":modules/output~.html so you can enjoy the wonderful 5 notes-based arpeggio composed for this example...). Similarly to the example aforementioned, we want to make sure that our synth is properly initialized before starting to play our sequence.  "jcom.initialized":components/initialized.html can then be used to send a @bang@ when the module defined as its argument is properly initialized. From then, the @bang@ message can trigger the message box that it is connected to and send to "jmod.qmetro /myClock":modules/qmetro.html the following couple of message to set a speed and to turn it on.
...
"Fine. But how does this really works?" Glad you ask.

"jcom.initialized":components/initialized.html is a Max patch-based component (_abstraction_) that wraps a special feature of "jcom.receive":components/receive.html. 

!images/jcom.intitialized_inside.png!

When provided @'notifications'@ as argument, "jcom.receive":components/receive.html can be used to notify of various operations done in Jamoma framework, i.e module creation, module deletion and more importantly here: module initialization. Thus, for each module in a patch, "jcom.receive notifications":components/receive.html will send the message @module.initialized@ followed by the class of the module and its instance name. In the previously used example, "jcom.receive":components/receive.html would send the following messages: 
...
Thus "jcom.initialized":components/initialized.html is a handy component that filters out notifications sent by "jcom.receive notifications":components/receive.html and sends a @bang@ when matching initialization notification for a given named module defined as argument.

h3(seealso). See also

* "jcom.init":components/init.html
* "jmod.initialization":modules/initialization.html
