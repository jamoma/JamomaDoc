...
h3. Installing Jamoma Modular development version




Jamoma source codes are maintained using "Git":http://git-scm.com/ "version control system":http://en.wikipedia.org/wiki/Revision_control and can be downloaded[1] freely from the public "Github":gh repository where they are hosted. In order to get Jamoma source code on your computer, first make sure you have Git installed and set up.

fn2. As the name suggests it, please note that development version is not for prime use and may contains some bugs being worked on.


h4(LessonTitle). <span class="StepNumber">1</span>Getting Git under OSX
...
!(lightbox)images/get_Git.png!


Follow the various steps of the installer.
!(lightbox)images/get_Git_2.png!


You now have Git installed on your computer ![2]

fn2. If you plan to use Git with a GUI client rather than from Terminal, you need to run the shell script provided in the disk image.
!(lightbox)images/get_Git_3.png!

h4(LessonTitle). <span class="StepNumber">2</span>Getting Git under Windows

Not yet available. Please ask for help on Jamoma forum.


h4(LessonTitle). <span class="StepNumber">3</span>Cloning Jamoma repository
...
!(lightbox)images/clone_as_user_3.png!
...
!(lightbox)images/update_submodule_1.png!
...
!(lightbox)images/update_submodule_2.png!



!(lightbox)images/update_submodule_3.png!

h4(LessonTitle). <span class="StepNumber">4</span>Building Jamoma framework and externals

Another Ruby script is provided to automate the building of all framework and Max externals. Assuming your are still running your Terminal session from the @Tools@ folder, execute the provided script by typing @ruby build.rb Development clean@ and hitting %(kbd)&#x21A9;%. The script will compile every component needed and copy them to their appropriate place.



Wait until the whole process is done. Please note that this can take quite a while.
!(lightbox)images/build_framework_2.png!


p(info). On your daily workflow, you will likely want to run the Ruby script @all.rb@ found in the @Tools@ folder which take care of running both the @update.rb@ and @build.rb@ scripts.
!(lightbox)images/build_framework_3.png!

h4(LessonTitle). <span class="StepNumber">5</span>Setting Max preferences

Launch Max 5 and in the @Options@ menu %(seq)1% choose the @File Preferences@ item %(seq)2%.



Then, in the File Preferences window, add the following paths to Max 5 search path :

- @~/Jamoma/builds/MaxMSP@
- @~/Jamoma/Modules/Modular/Max@
- @~/Jamoma/UserLib@ (if you plan to use the user contributed modules)
!(lightbox)images/Max_pref_setup.png!


Max 5 objects default appearance also need to be set up so all Jamoma modules are displayed as expected. From Max 5 @Options@ menu %(seq)1%, choose the @Object Defaults@ item %(seq)2%



In the @Object Defaults@ window, click the @Load Settings@ button in the lower-left corner of the window %(seq)1%. Then, from the popup menu, select @JamomaMax@ %(seq)2% so all Jamoma modules will be displayed as expected.
!(lightbox)images/media_1283896186931.png!
...
!(lightbox)images/Max_obj_setup_3.png!
