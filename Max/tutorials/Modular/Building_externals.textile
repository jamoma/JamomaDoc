...
bq. For users incomfortable with the Terminal, just type the commend @cd@ followed by a space and drop the Tools folder from the Finder to the Terminal window.

To build the whole Jamoma projects (i.e. all its subprojects), use the provided Ruby script by running
...
This will and build externals, libraries and frameworks. You may have to do the last step twice to get everything to build.

bq. There are also scripts to build only one Jamoma sub-project at a time (e.g. only DSP or modular),
to do so, navigate to it to it, e.g. @cd /Modules/DSP@ to build the DSP framework and externals, and then run its specific build.rb script by running (while being in its folder):
