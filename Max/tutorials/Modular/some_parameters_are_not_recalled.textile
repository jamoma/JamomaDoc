...
objref: [j.parameter, jmod.degrade~]
---
...
Another cause for the asked problem may be related to one of the "j.parameter":components/j.parameter.html options. Indeed, a particular attribute which is often overseen is the @repetitions/allow@ attribute. This attribute (set to @0@ by default) defined whether "j.parameter":components/j.parameter.html should trigger its output when receiving a value identical to the current one. For example, assuming you built a sampler module containing a "j.parameter example/play @repetitions/allow 0":components/j.parameter.html with its value set to @1@, sending %(maxMessage)/example/play 1% to your module will cause no output from your "j.parameter":components/j.parameter.html. It will only trigger its output when sending a value different than @1@ (in this case). On the contrary, when @repetitions/allow@ is set to @1@, sending %(maxMessage)/example/play 1% twice will cause "j.parameter":components/j.parameter.html to output its value each time the message is received.